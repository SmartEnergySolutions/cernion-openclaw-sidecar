...
function validateReadOnlyApiGetPath(path) {
...
function scoreOpenApiOperation(operation, queryTokens) {
...
function toOpenApiFallbackCandidate(operation, score) {
    const method = String(operation.method || "").toUpperCase();
    const path = String(operation.path || "");
    ...
    try {
        if (executeSupported)
            validateReadOnlyApiGetPath(path);
    }
    catch (error) {
...
function fillPathParams(path, params) {
    ...
    const pathWithParams = path.replace(/:([A-Za-z0-9_]+)/g, (_match, name) => {
...
async function resolveOpenApiFallback(config, request, signal) {
    ...
    const response = {
        ...
        answerGuidance: "Use this only as a generic read-only OpenAPI fallback after curated Cernion capabilities, Evidence Router, or cernion.ask fail to return a deterministic plan. Prefer selected.path only when executeSupported=true; POST or blocked operations need a curated Evidence Router policy plan.",
    };
    if (request.execute !== true || !executableCandidate)
        return response;
    const { path, params } = fillPathParams(executableCandidate.path, request.params || {});
    const result = await executeRestExecutionPlan(config, {
        method: "GET",
        path,
        params,
    }, signal);
...
export default defineToolPlugin({
    ...
    tools: (tool) => [
        ...
        tool({
            ...
            execute: async (params, config, context) => {
                const result = await resolveOpenApiFallback(config, params, context.signal);
                return scrubSecretValues(result, config.bearerToken);
            },
        }),
        tool({
