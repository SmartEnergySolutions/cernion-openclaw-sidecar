...
- `cernion_query_domain_knowledge` queries Cernion Knowledge RAG for regulatory, procedural, and fachliche evidence such as laws, BNetzA guidance, Verfahrensanweisungen, roles, obligations, definitions, and job-help context. It starts the async Knowledge RAG job, waits briefly for the result, and returns an `evidenceAssessment`. If `evidenceAdequacy` is `low`, OpenClaw should not present legal or procedural duties as fully evidenced by Cernion and should name the evidence gap instead of filling it from model memory.
